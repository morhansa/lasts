...
public function scanCssForFonts($cssFilePath)
{
    ...
    try {
        ...
        // Match font URLs in the CSS - Improve regular expressions to capture more fonts
        $fontPatterns = [
         // General syntax for fonts in any url() within CSS
            '/url\([\'"]?(\/[^"\'\)]+\.(woff2?|eot|ttf|otf))[\'"]?\)/i',
            
        // Search for @font-face specifically
            '/@font-face\s*{[^}]*src:\s*url\([\'"]?(\/[^"\'\)]+\.(woff2?|eot|ttf|otf))[\'"]?\)/i',
            
       // Find absolute URLs with domain
            '/url\([\'"]?(https?:\/\/[^"\'\)]+\.(woff2?|eot|ttf|otf))[\'"]?\)/i',
            
          //Alternate font string format (multiple format)
            '/src:\s*(?:local\([^\)]+\),\s*)*url\([\'"]?(\/[^"\'\)]+\.(woff2?|eot|ttf|otf))[\'"]?\)/i',
            
      // Find fonts in any popular style
            '/[:\s]url\([\'"]?([^"\'\)]+\.(woff2?|eot|ttf|otf))[\'"]?\)/i'
        ];
        ...
        foreach ($fontPatterns as $pattern) {
            if (preg_match_all($pattern, $cssContent, $matches)) {
                ...
                // URL sanitization and normalization
                    $fontUrl = $this->normalizeFontUrl($fontUrl);
                    if (!empty($fontUrl)) {
                        $fontUrls[] = $fontUrl;
                    }
                }
...
/**
* Normalize URL to font
 * 
 * @param string $url
 * @return string
 */
private function normalizeFontUrl($url)
{
    // Skip data URLs
