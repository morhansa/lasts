...
class AnalyzeUrls extends Action
{
    ...
    protected function extractUrls($content)
    {
        $urls = [];
		   ...
		   // New style for searching for fonts in preloads
    $preloadPattern = '/<link\s+[^>]*rel=[\'"]preload[\'"][^>]*as=[\'"]font[\'"][^>]*href=[\'"]([^"\']+)[\'"][^>]*>/i';
    if (preg_match_all($preloadPattern, $content, $matches)) {
        foreach ($matches[1] as $fontUrl) {
          // URL normalization
            $url = $this->normalizeUrl($fontUrl);
    ...
    // Find any link tag that points to a line
    $fontLinkPattern = '/<link\s+[^>]*href=[\'"]([^"\']+\.(woff2?|eot|ttf|otf))[\'"][^>]*>/i';
    if (preg_match_all($fontLinkPattern, $content, $matches)) {
        foreach ($matches[1] as $fontUrl) {
            $url = $this->normalizeUrl($fontUrl);
            if ($this->isValidUrl($url)) {
                $urls[] = $url;
